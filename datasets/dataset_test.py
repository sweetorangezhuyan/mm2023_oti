...
#         self._parse_list()
    ...
    def _get_test_indices(self,video_list):
        ...
        # random sample
        indexs = sorted(np.random.choice(len(sample_set), self.num_segments * num_clips, replace=False))

        # uniform sample
        # all_f=self.num_segments* num_clips
        # inter=len(sample_set)//all_f
        # indexs=np.arange(len(sample_set))[::inter][:all_f]
