...
from modules.oti_zsvr import video_header, VideoCLIP
...
def main(args):
    ...
    working_dir = os.path.join('records/exp_few_shot', config['data']['dataset'], config['network']['arch'],args.log_time)
    if dist.get_rank() == 0:
...
#         shutil.copy('train_k400_two_ce_loss.py', working_dir)
    ...
    val_data = Video_dataset(
            ...
            modality=config.data.modality,
            image_tmpl=config.data.image_tmpl, random_shift=config.data.random_shift,
            transform=transform_val,new_length=config.data.seg_length)
...
#         images= images.view(-1, c, h, w)
        with autocast():
            ...
            image_emb,image_emb_front,logits_new,logits_after = model(images, text_embedding) # B  400
...
def validate(epoch, val_loader, device, model, config, text_embedding, logger):
    ...
    with torch.no_grad():
        for i, (images, class_id) in enumerate(val_loader):
            ...
            image_embedding ,front_image_emb= model.module.encode_image(images)
