...
def init_distributed_mode(args):
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        args.rank = int(os.environ["RANK"])## 用于表示进程的编号/序号 每一个进程对应了一个rank的进程，整个分布式由许多rank完成。
        args.world_size = int(os.environ['WORLD_SIZE'])## 节点个数*GPU数量，rank的个数，GPU的数量
        args.gpu = int(os.environ['LOCAL_RANK'])## local_rank是指在一个node上进程的相对序号
    ...
    args.dist_backend = 'nccl'## 通信后端：主要完成模型训练过程中参数信息的传递，主要考虑通信后端和通信模式选择
    '''
    ...
    torch.distributed.barrier()## 等待同步操作，对所以的进程进行同步


def ddp_all_reduce(*args):## 将不同rank进程的数据进行操作
