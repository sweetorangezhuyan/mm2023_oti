...
def main(args):
    ...
    model_full=OTI_TPT(clip_model,video_head,val_classes,config.data)
    '''
    参数加载
    args.weights：参数
    '''
    if os.path.isfile(args.weights):
        ...
        model_full.load_state_dict(update_dict(checkpoint['model_state_dict']),strict=False)
        del checkpoint
    ...
    prec1 = validate(
        val_loader, device,
        model_full, config, val_classes,args.test_crops, args.test_clips,save_root)
    print(prec1)
    return


def validate(val_loader, device, model, config,val_classes, test_crops, test_clips,save_root):
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        n_class = len(val_classes)
        ## text feature
        promptes=model.module.prompt_learner()
        tokenized_prompts=model.module.tokenized_prompts
        text_features=model.module.textual(promptes,tokenized_prompts)
...
#             print(b, t, c, h, w )
            ...
            image_features,front_image_emb = model.module.encode_videos(image)
