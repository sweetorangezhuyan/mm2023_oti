...
from modules.video_senet import VideoCLIP
...
#     classes_features=torch.
    ...
    model_full=VideoCLIP(clip_model,config.data.num_segments)#video_head
...
#         images= images.view(-1, c, h, w)
        with autocast():
            ...
            logits_after = model(images, text_embedding) # B  400

            # loss1 = criterion(logits_new, list_id.to(device))
            loss=criterion(logits_after,list_id.to(device))
