...
def get_orthogonal_num_vec(now_tensor,best_tensor):
    ...
    return res
def get_orthogonal_gain(now_tensor,best_tensor):
    l2sums=torch.norm(best_tensor,dim=1)**2
    l2sums=torch.unsqueeze(l2sums,dim=1)
    maps=(now_tensor*best_tensor)/l2sums
    maps*=best_tensor 
...
#         self.ratio.requires_grad=True
        ...
        self.dim=clip_model.ln_final.bias.shape[0]
        self.le_a=torch.nn.Parameter(torch.randn(self.dim,16))
        # self.le_re=torch.nn.ReLU()
        self.le_b=torch.nn.Parameter(torch.zeros(16,self.dim))
        # self.le_or_low_rank=nn.Sequential(
        #     nn.Linear(self.dim,16),
        #     nn.ReLU(),
        #     nn.Linear(16,self.dim))
        # self.lnor=nn.LayerNorm(self.dim)
    ...
    def forward(self, image, text_emb):
        ...
        gain_vector=get_orthogonal_gain(image_emb,image_emb_front)# bs*dim
        #new_gain_vector=self.le_or_low_rank(gain_vector)+gain_vector#+gain_vector@self.le_a@self.le_b
        new_gain_vector=gain_vector@self.le_a@self.le_b
        res=new_gain_vector+image_emb_front#+gain_vector
        new_vector=res/res.norm(dim=-1, keepdim=True)
